...
const AskImamSettingScreen = ({ navigation, route }) => {
  ...
  const getAskImam = async () => {
    try {
      const result = await fetch("https://community.sadathussain.com/api/post?category_id=1", {
        method: 'post'
      })
    ...
    <BgImage>
      <View style={styles.container}>
        ...
        <ScrollView
          ...
          <><Image
            source={{ uri: askImam.thumbnail }}
            style={{ width: '100%', height: 200 }} /><View style={{ width: '100%', paddingHorizontal: 20 }}>
              <Text style={styles.heading}>{askImam.title}</Text>
              ...
              <Text
                ...
                {' '}
                {askImam.description?.replace(regex, '')}
              </Text>
            </View>
            ...
            </View><View style={styles.inContainer1}>
              <Text
                style={{
                  color: '#a7c829',
                  fontSize: 18,
...
export default AskImamSettingScreen;
